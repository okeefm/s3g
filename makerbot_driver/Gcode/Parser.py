...
class GcodeParser(object):
  ...
  def execute_line(self, command):
    ...
    try:
      command = makerbot_driver.Gcode.Utils.variable_substitute(command, self.environment) 

      codes, flags, comment = makerbot_driver.Gcode.Utils.parse_line(command)

      if 'G' in codes:
        if codes['G'] in self.GCODE_INSTRUCTIONS:
          makerbot_driver.Gcode.Utils.check_for_extraneous_codes(codes.keys(), self.GCODE_INSTRUCTIONS[codes['G']][1])
          makerbot_driver.Gcode.Utils.check_for_extraneous_codes(flags, self.GCODE_INSTRUCTIONS[codes['G']][2])
        ...
        if codes['M'] in self.MCODE_INSTRUCTIONS:
          makerbot_driver.Gcode.Utils.check_for_extraneous_codes(codes.keys(), self.MCODE_INSTRUCTIONS[codes['M']][1])
          makerbot_driver.Gcode.Utils.check_for_extraneous_codes(flags, self.MCODE_INSTRUCTIONS[codes['M']][2])
      ...
      # Not a G or M code, should we throw here?
        ...
        if len(codes) + len(flags) > 0:
          ...
          raise makerbot_driver.Gcode.gcode_error
  ...
  def set_potentiometer_values(self, codes, flags, comment):
    ...
    for a in makerbot_driver.Gcode.Utils.parse_out_axes(codes):
  ...
  def find_axes_maximums(self, codes, flags, command):
    ...
    """
    axes = makerbot_driver.Gcode.Utils.parse_out_axes(flags)
    if len(axes) == 0:
      return
    ...
    dda_speed = makerbot_driver.Gcode.Utils.calculate_homing_DDA_speed(
        codes['F'], 
        axes_feedrates,
        axes_SPM
        )
  ...
  def set_position(self, codes, flags, comment):
    ...
    stepped_position = makerbot_driver.Gcode.Utils.multiply_vector(self.state.get_position(), self.state.get_axes_values('steps_per_mm'))
  ...
  def disable_axes(self, codes, flags, comment):
    ...
    """
    self.s3g.toggle_axes(makerbot_driver.Gcode.Utils.parse_out_axes(flags), False)
  ...
  def linear_interpolation(self, codes, flags, comment):
    ...
    if len(makerbot_driver.Gcode.Utils.parse_out_axes(codes)) > 0 or 'E' in codes:
      ...
      try :
        ...
        dda_speed = makerbot_driver.Gcode.Utils.calculate_DDA_speed(
            current_position, 
        ...
        stepped_point = makerbot_driver.Gcode.Utils.multiply_vector(
            self.state.get_position(), 
            self.state.get_axes_values('steps_per_mm')
            )
