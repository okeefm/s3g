...
def encode_payload(payload):
    ...
    if len(payload) > constants.maximum_payload_length:
        raise errors.PacketLengthError(
            len(payload), constants.maximum_payload_length)
...
def decode_packet(packet):
    ...
    if packet[len(packet) - 1] != CalculateCRC(packet[2:(len(packet) - 1)]):
        raise errors.PacketCRCError(packet[len(
            packet) - 1], CalculateCRC(packet[2:(len(packet) - 1)]))
    ...
    def parse_byte(self, byte):
        ...
        if self.state == 'WAIT_FOR_HEADER':
            ...
            if byte > constants.maximum_payload_length:
                raise errors.PacketLengthFieldError(
                    byte, constants.maximum_payload_length)
