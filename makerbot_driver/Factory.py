...
def create_eeprom_reader(port, firmware_verison=5.6, working_directory=None):
    ...
    reader = makerbot_driver.EEPROM.EepromReader.factory(s3g, firmware_verison, working_directory)
    return reader

def create_eeprom_writer(port, firmware_version=5.6, working_directory=None):
    s3g = makerbot_driver.s3g.from_filename(port) 
    writer = makerbot_driver.EEPROM.EepromWriter.factory(s3g, firmware_version, working_directory)
    return writer
