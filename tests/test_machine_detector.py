...
class TestMachineDetectorScanTests(unittest.TestCase):
  ...
  def test_scan_no_new_bots(self):
    ...
    expected_just_seen = []
    self.assertEqual(self.md.machines_recently_seen, expected_recently_seen)
  ...
  def test_scan_new_bots_additional_bot(self):
    ...
    def return_func(*args, **kwards):
      return [blobs.pop()]
    self.list_ports_mock.side_effect = return_func
    self.md.scan()
    self.assertEqual(md.machines_just_seen, [blob1])
    self.assertEqual(md.machines_recently_seen, [blob1])
    self.md.scan()
    self.assertEqual(md.machines_just_seen, [blob2])
    self.assertEqual(md.machines_recently_seen, [blob1, blob2])
