...
class TestFromFileName(unittest.TestCase):
    def setUp(self):
        self.obj = makerbot_driver.s3g.from_filename(None)
...
class S3gTestsFirmware601(unittest.TestCase):
    ...
    """
    def setUp(self):
        self.r = makerbot_driver.s3g(firmware_version=601)
        ...
        writer = makerbot_driver.Writer.StreamWriter(file)
...
class S3gTestsFirmware500(unittest.TestCase):
    ...
    """
    def setUp(self):
        self.r = makerbot_driver.s3g(firmware_version=500)
        self.outputstream = io.BytesIO(
        )  # Stream that we will send responses on
        self.inputstream = io.BytesIO(
        )  # Stream that we will receive commands on
    ...
    def test_set_servo_2_position(self):
        ...
        self.assertEqual(payload[2], makerbot_driver.slave_action_command_dict[
    ...
    def test_toggle_abp(self):
        ...
        self.assertEqual(payload[1], tool)
        self.assertEqual(
            payload[2], makerbot_driver.slave_action_command_dict['TOGGLE_ABP'])
        self.assertEqual(payload[3], 1)
    ...
    def test_get_toolcount(self):
        ...
        self.assertEqual(
            payload[1:3], makerbot_driver.Encoder.encode_uint16(eeprom_offset_toolcount))
    ...
    def test_get_version(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['GET_VERSION'])
        self.assertEqual(
            payload[1:3], makerbot_driver.Encoder.encode_uint16(makerbot_driver.makerbot_driver.s3g_version))
    ...
    def test_get_advanced_version(self):
        ...
        packet = bytearray(self.inputstream.getvalue())
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEqual(payload[0], makerbot_driver.host_query_command_dict[
    ...
    def test_reset(self):
        ...
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['RESET'])
    ...
    def test_is_finished(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['IS_FINISHED'])
    ...
    def test_clear_buffer(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['CLEAR_BUFFER'])
    ...
    def test_pause(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['PAUSE'])

    def test_tool_query_negative_tool_index(self):
        self.assertRaises(
            makerbot_driver.ToolIndexError,
            self.r.tool_query,
            -1,
            makerbot_driver.slave_query_command_dict['GET_VERSION']
        )
    ...
    def test_tool_query_too_high_tool_index(self):
        self.assertRaises(
            makerbot_driver.ToolIndexError,
            self.r.tool_query,
            makerbot_driver.max_tool_index + 1,
            makerbot_driver.slave_query_command_dict['GET_VERSION']
        )
    ...
    def test_tool_query_no_payload(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEquals(
            payload[0], makerbot_driver.host_query_command_dict['TOOL_QUERY'])
    ...
    def test_read_from_eeprom_bad_length(self):
        offset = 1234
        length = makerbot_driver.maximum_payload_length

        self.assertRaises(
            makerbot_driver.EEPROMLengthError, self.r.read_from_EEPROM, offset, length)

    def test_read_from_eeprom(self):
        offset = 1234
        length = makerbot_driver.maximum_payload_length - 1
        data = bytearray()
    ...
    def test_write_to_eeprom_too_much_data(self):
        offset = 1234
        length = makerbot_driver.maximum_payload_length - 3
        data = bytearray()
        for i in range(0, length):
            data.append(i)

        self.assertRaises(
            makerbot_driver.EEPROMLengthError, self.r.write_to_EEPROM, offset, data)

    def test_write_to_eeprom_bad_response_length(self):
        offset = 1234
        length = makerbot_driver.maximum_payload_length - 4
        data = bytearray()
        for i in range(0, length):
            data.append(i)
        ...
        self.assertRaises(
            makerbot_driver.EEPROMMismatchError, self.r.write_to_EEPROM, offset, data)
    ...
    def test_get_available_buffer_size(self):
        ...
        packet = bytearray(self.inputstream.getvalue())
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEquals(payload[0], makerbot_driver.host_query_command_dict[
    ...
    def test_playback_capture_error_codes(self):
        ...
        for error_code in makerbot_driver.sd_error_dict:
            if error_code != 'SUCCESS':
                ...
                self.assertRaises(
                    makerbot_driver.SDCardError, self.r.playback_capture, filename)
    ...
    def test_playback_capture(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEquals(
            payload[0], makerbot_driver.host_query_command_dict['PLAYBACK_CAPTURE'])
    ...
    def test_get_next_filename_error_codes(self):
        for error_code in makerbot_driver.sd_error_dict:
            if error_code != 'SUCCESS':
                ...
                self.assertRaises(
                    makerbot_driver.SDCardError, self.r.get_next_filename, False)
    ...
    def test_get_build_name(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEquals(
            payload[0], makerbot_driver.host_query_command_dict['GET_BUILD_NAME'])
    ...
    def test_wait_for_button_bad_button(self):
        button = 'bad'
        self.assertRaises(makerbot_driver.ButtonError, self.r.wait_for_button,
    ...
    def test_queue_song(self):
        ...
        self.assertEqual(
            payload[0], makerbot_driver.host_action_command_dict['QUEUE_SONG'])
    ...
    def test_display_message(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEquals(
            payload[0], makerbot_driver.host_action_command_dict['DISPLAY_MESSAGE'])
    ...
    def test_build_start_notification_long_name(self):
        ...
        max_build_name_length = makerbot_driver.maximum_payload_length - 7
        ...
        self.assertEquals(
            payload[1:5], makerbot_driver.Encoder.encode_uint32(0))  # Reserved uint32
    ...
    def test_find_axes_minimums(self):
        ...
        self.outputstream.write(
            makerbot_driver.Encoder.encode_payload([makerbot_driver.response_code_dict['SUCCESS']]))
        self.outputstream.seek(0)
    ...
    def test_set_extended_position(self):
        ...
        for i in range(0, 5):
            self.assertEquals(makerbot_driver.Encoder.encode_int32(
    ...
    def test_get_toolhead_version(self):
        ...
        self.assertEquals(payload[1], tool_index)
        self.assertEquals(
            payload[2], makerbot_driver.slave_query_command_dict['GET_VERSION'])
        self.assertEquals(
            payload[3:5], makerbot_driver.Encoder.encode_uint16(makerbot_driver.makerbot_driver.s3g_version))
    ...
    def test_capture_to_file(self):
        ...
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['CAPTURE_TO_FILE'])
        self.assertEqual(payload[1:-1], filename)
        self.assertEqual(payload[-1], 0x00)

    def test_capture_to_file_makerbot_driver(self):
        filename = 'test'
        ...
        for error_code in makerbot_driver.sd_error_dict:
            if error_code != 'SUCCESS':
                ...
                self.assertRaises(
                    makerbot_driver.SDCardError, self.r.capture_to_file, filename)
    ...
    def test_end_capture_to_file(self):
        ...
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['END_CAPTURE'])
    ...
    def test_store_home_positions(self):
        ...
        self.assertEqual(payload[1], makerbot_driver.Encoder.encode_axes(axes))
    ...
    def test_set_rgb_led(self):
        ...
        self.assertEqual(
            payload[0], makerbot_driver.host_action_command_dict['SET_RGB_LED'])
    ...
    def test_set_potentiometer_value(self):
        ...
        self.assertEqual(
            payload[0], makerbot_driver.host_action_command_dict['SET_POT_VALUE'])
        self.assertEqual(payload[1], makerbot_driver.Encoder.encode_axis(axes))
    ...
    def test_queue_extended_point_new_short_length(self):
        ...
        self.assertRaises(makerbot_driver.PointLengthError, self.r.queue_extended_point_new, point, duration, relative_axes)
    ...
    def test_toggle_axes(self):
        ...
        bitfield = makerbot_driver.Encoder.encode_axes(axes)
        ...
        self.assertEqual(
            payload[0], makerbot_driver.host_action_command_dict['ENABLE_AXES'])
    ...
    def test_change_tool(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEqual(
            payload[0], makerbot_driver.host_action_command_dict['CHANGE_TOOL'])
    ...
    def test_get_build_stats(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['GET_BUILD_STATS'])
    ...
    def test_extended_stop_error(self):
        ...
        self.assertRaises(
            makerbot_driver.ExtendedStopError, self.r.extended_stop, True, True)
    ...
    def test_get_pid_state(self):
        ...
        self.assertEqual(
            payload[2], makerbot_driver.slave_query_command_dict['GET_PID_STATE'])
    ...
    def test_toolhead_init(self):
        ...
        self.assertEqual(payload[1], toolIndex)
        self.assertEqual(
            payload[2], makerbot_driver.slave_action_command_dict['INIT'])
    ...
    def test_toolhead_abort(self):
        ...
        self.assertEqual(payload[1], toolIndex)
        self.assertEqual(
            payload[2], makerbot_driver.slave_action_command_dict['ABORT'])
    ...
    def test_toolhead_pause(self):
        ...
        self.assertEqual(payload[1], toolIndex)
        self.assertEqual(
            payload[2], makerbot_driver.slave_action_command_dict['PAUSE'])
        self.assertEqual(payload[3], len(bytearray()))
        self.assertEqual(payload[4:], bytearray())
    ...
    def test_toggle_motor_1(self):
        ...
        self.assertEqual(payload[1], toolIndex)
        self.assertEqual(
            payload[2], makerbot_driver.slave_action_command_dict['TOGGLE_MOTOR_1'])
        self.assertEqual(payload[3], len(expectedPayload))
        self.assertEqual(payload[4:], expectedPayload)
    ...
    def test_set_motor_1_speed_rpm(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        expectedPayload = bytearray()
        expectedPayload.extend(makerbot_driver.Encoder.encode_uint32(duration))
    ...
    def test_get_tool_status(self):
        ...
        self.assertEqual(payload[1], toolIndex)
        self.assertEqual(
            payload[2], makerbot_driver.slave_query_command_dict['GET_TOOL_STATUS'])
    ...
    def test_get_motor_speed(self):
        ...
        self.assertEqual(
            payload[0], makerbot_driver.host_query_command_dict['TOOL_QUERY'])
        self.assertEqual(payload[1], toolIndex)
        self.assertEqual(payload[2], makerbot_driver.slave_query_command_dict[
    ...
    def test_init(self):
        ...
        payload = makerbot_driver.Encoder.decode_packet(packet)
        self.assertEquals(
            payload[0], makerbot_driver.host_query_command_dict['INIT'])
    ...
    def test_is_tool_ready_bad_response(self):
        ...
        self.assertRaises(
            makerbot_driver.HeatElementReadyError, self.r.is_tool_ready, tool_index)
    ...
    def test_read_from_toolhead_eeprom_bad_length(self):
        ...
        self.assertRaises(makerbot_driver.EEPROMLengthError, self.r.read_from_toolhead_EEPROM, tool_index, offset, length)
    ...
    def test_write_to_toolhead_eeprom_too_much_data(self):
        tool_index = 2
        offset = 1234
        length = makerbot_driver.maximum_payload_length - 5
        data = bytearray()
        for i in range(0, length):
            data.append(i)

        self.assertRaises(makerbot_driver.EEPROMLengthError, self.r.write_to_toolhead_EEPROM, tool_index, offset, data)

    def test_write_to_toolhead_eeprom_bad_response_length(self):
        tool_index = 2
        offset = 1234
        length = makerbot_driver.maximum_payload_length - 6
        data = bytearray()
        for i in range(0, length):
            data.append(i)
    ...
    def test_toggle_fan(self):
        ...
        for fan_state in fan_states:
            ...
            self.outputstream.write(makerbot_driver.Encoder.encode_payload(
                [makerbot_driver.response_code_dict['SUCCESS']]))
            self.outputstream.seek(0)
            self.inputstream.seek(0)
            ...
            self.assertEquals(payload[1], tool_index)
            self.assertEquals(
                payload[2], makerbot_driver.slave_action_command_dict['TOGGLE_FAN'])
