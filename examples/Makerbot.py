...
class Makerbot:
	...
	def Send(self, func, *args):
		try:
			return func(*args)
		except s3g.TransmissionError:
			print "Packet Lost, resending..."
			return self.Send(func, *args)
	...
	def GetToolheadTemperature(self, toolhead):
		if not self.HasToolhead(toolhead):
			...
			return self.Send(self.s3g.GetToolheadTemperature, toolhead)

	def GetPlatformTemperature(self):
		return self.Send(self.s3g.GetPlatformTemperature, 0)

	def SetExtruderTemperature(self, toolhead, target):
		if not self.HasToolhead(toolhead):
			raise s3g.ProtocolError('Tool index out of range, got=%i, max=%i'%(tool_index, max_tool_index))
		return self.Send(self.s3g.SetToolheadTemperature, toolhead, target)

	def SetPlatformTemperature(self, target):
		return self.Send(self.s3g.SetPlatformTemperature, 0, target)

	def GetVersion(self):
		return self.Send(self.s3g.GetVersion)
	...
	def GetExtruderPID(self):
		...
		for offset in pid_offsets:
			value = ((self.Send(self.s3g.ReadFromEEPROM, pid_base, offset)))
			vals.append(s3g.DecondUint16(value))
		return vals
